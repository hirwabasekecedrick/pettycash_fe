...
interface Payment {
  ...
  assignment?: {
    ...
    authorizedItems: { id: string; name: string }[];
  };
}
...
export default function TransactionsPage() {
  ...
  const filtered = payments.filter(p =>
    ...
    (p.assignment?.assignedTo?.name || '').toLowerCase().includes(search.toLowerCase())
  );

  const totalSpent = filtered.reduce((s, p) => s + p.amount, 0);

  const groupedPayments = groupByAssignment ? filtered.reduce((acc, p) => {
    const key = p.assignment ? `Assignment to ${p.assignment.assignedTo.name} (RWF ${p.assignment.amount})` : 'Unassigned';
    if (!acc[key]) acc[key] = [];
    acc[key].push(p);
    return acc;
  }, {} as Record<string, Payment[]>) : { 'All': filtered };
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search by reason, vendor, or wallet account holder..."
              value={search}
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-colors border ${groupByAssignment ? 'bg-primary text-white border-primary' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}
          >
            Group by Assignment
          </button>
        </div>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-50">
              {Object.entries(groupedPayments).map(([group, groupPayments]) => (
                <div key={group}>
                  {groupByAssignment && (
                    <div className="bg-gray-100 px-5 py-2.5 text-xs font-bold text-gray-600 uppercase tracking-wider">
                      {group}
                    </div>
                  )}
                  {groupPayments.map(p => (
                      ...
                      {expanded === p.id && (
                        <div className="px-5 pb-5 bg-gray-50/50 border-t border-gray-100">
                          <div className="pt-4 space-y-3">
                            ...
                            {p.employee && (
                              <div>
                                <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Wallet Account Holder</p>
                            ...
                            {p.assignment && (
                              <div>
                                <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Assignment Details</p>
                                <p className="text-sm text-gray-700 mt-1">Assigned to: {p.assignment.assignedTo.name}</p>
                                <p className="text-sm text-gray-700">Total Assigned: RWF {p.assignment.amount.toLocaleString()}</p>
                                {p.assignment.authorizedItems && p.assignment.authorizedItems.length > 0 && (
                                  <p className="text-sm text-gray-700">Authorized: {p.assignment.authorizedItems.map(i => i.name).join(', ')}</p>
                                )}
                              </div>
                            )}
                            {p.images.length > 0 && (
